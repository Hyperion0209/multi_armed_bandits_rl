...
def plotter(x, y, what_this_is):
  plt.plot(x, y)
  title = "Regrets vs Arm 2's Mean (P2)"
  # if what_this_is == "Task2A":
  #   title = "Regrets vs Arm 2's Mean (P2)"
  # elif what_this_is == "Task2B_ucb"
  #   title = 
  # elif what_this_is == "TAsk3B_klucb":
  plt.title(title)
  plt.savefig("task1-{}-{}.png".format(what_this_is, time.strftime("%Y%m%d-%H%M%S")))
  plt.clf()
# END EDITING HERE
...
if __name__ == '__main__':
  # EXAMPLE CODE
  # task2p1s = [0.9, 0.9, 0.9]
  # task2p2s = [0,0.5, 0.9]
  task2Ap2s = np.arange(0, 0.901, 0.05)
  task2Ap1s = np.full((task2Ap2s.shape), 0.9)
  print(type(task2Ap1s.shape[0]))
  # regrets = task2(Eps_Greedy, 30000, task2p1s, task2p2s)
  task2Bp2s = task2Ap2s
  task2Bp1s = np.add(task2Bp2s, 0.1)
  regrets_ucb_2A = task2(UCB, 30000, task2Ap1s, task2Ap2s)
  regrets_ucb_2B = task2(UCB, 30000, task2Bp1s, task2Bp2s)
  regrets_kl_ucb_2B = task2(KL_UCB, 30000, task2Bp1s, task2Bp2s)
  # # print(regrets)
  # print(regrets_ucb)
  plotter(task2Ap2s, regrets_ucb_2A, "Task2A")
  plotter(task2Bp2s, regrets_ucb_2B, "Task2B_UCB")
  plotter(task2Bp2s, regrets_kl_ucb_2B, "Task2B_KLUCB")

  plt.plot(task2Bp2s, regrets_ucb_2B, label='Graph 1', linestyle='-', marker='o', color='b')
  plt.plot(task2Bp2s, regrets_kl_ucb_2B, label='Graph 2', linestyle='--', marker='x', color='r')
  plt.ylabel("Regrets")
  plt.xlabel("Arm 2's Mean (P2)")
  plt.legend()
  plt.show()
  # # plt.show()
  # plt.title("Regrets vs Arm 2's Mean (P2)")
  # plt.savefig("task1-{}-{}.png".format("Task2A", time.strftime("%Y%m%d-%H%M%S")))
  # plt.clf()
